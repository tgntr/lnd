...
func NewMiner(t *testing.T, extraArgs []string, createChain bool,
	...
	err = node.SetUp(createChain, spendableOutputs)
	require.NoError(t, err, "unable to set up backend node")
...
func NewBitcoindBackend(t *testing.T, minerAddr string, txindex,
	...
	require.NoError(t, err, "unable to establish connection to bitcoind")
	t.Cleanup(conn.Stop)
...
func NewNeutrinoBackend(t *testing.T, minerAddr string) *neutrino.ChainService {
	...
	require.NoError(t, err, "unable to create walletdb")
	t.Cleanup(func() {
		spvDatabase.Close()
	})
	...
	require.NoError(t, err, "unable to create neutrino")
